import { PhoneCall, MessageSquareText, Mail, Globe } from "lucide-react";

export default function ContactCTAContainer({ title, subtitle }) {
  return (
    <section className="w-full bg-transparent font-poppins py-10 sm:py-14">
      <div className="mx-auto max-w-7xl px-3 sm:px-6 lg:px-10">
        {/* Card */}
        <div className="mx-auto w-full max-w-[900px] rounded-2xl bg-[#2F2F2F] p-4 sm:p-8 lg:p-10 shadow-[0_18px_50px_rgba(15,23,42,0.18)]">
          {/* Top content */}
          <div className="max-w-[650px]">
            <h3 className="text-white font-bold text-[16px] sm:text-[24px] lg:text-[26px] leading-snug">
              {title}
            </h3>
  ...
  return (
    ...
    >
      <div className="h-10 w-10 sm:h-12 sm:w-12 rounded-lg flex items-center justify-center shrink-0">
        <Icon className="h-5 w-5 sm:h-6 sm:w-6 text-[#0B3BFF]" />
      </div>

      {/* allow text to shrink properly */}
      <div className="min-w-0">
        <div className="text-[14px] sm:text-[16px] font-bold tracking-wide text-[#111827]">
          {label}
        </div>

        {/* prevent overflow on 300px screens */}
        <div className="mt-0.5 text-[12px] sm:text-[13px] font-bold text-gray-600 truncate sm:whitespace-nowrap whitespace-normal break-all sm:break-normal">
          {value}
        </div>
      </div>
    </a>
