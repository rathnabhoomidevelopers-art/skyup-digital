...
const HeroSection = () => {
  ...
  const validate = () => {
    ...
    else if (!phoneRegex.test(form.mobile.trim())) next.mobile = "Enter a valid phone number";
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      setFormStatus({ type: "success", message: data?.message || "Submitted successfully!" });
  ...
  useEffect(() => {
    if (!auditOpen) return;
    const onKeyDown = (e) => { if (e.key === "Escape") setAuditOpen(false); };
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      tl.from(titleRef.current, { opacity: 0, y: 40, duration: 0.8 })
        ...
        .from([primaryBtnRef.current, secondaryBtnRef.current], { opacity: 0, y: 15, duration: 0.4, stagger: 0.1 }, "-=0.3");
    }, sectionRef);
    return () => ctx.revert();
  }, []);
        ...
        <div className="relative flex min-h-[60vh] md:min-h-[65vh] lg:min-h-[80vh] w-full items-center">
          <div className="mx-auto w-full max-w-6xl px-4 py-10 md:py-8 lg:py-16 sm:px-6 lg:px-16">
            <div className="space-y-6 text-center flex flex-col items-center">
              ...
              <div ref={ctaRef} className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 sm:gap-5 pt-3">
                <a
                  href="/contactus"
                  ref={primaryBtnRef}
          ...
          >
            ...
            >
              <div className="flex items-start justify-between gap-3 px-4 sm:px-6 py-3 sm:py-4 border-b">
                <div>
                  <h3 className="text-[18px] sm:text-[22px] font-bold text-slate-900">Get Free Audit</h3>
              ...
              <div className="px-4 sm:px-6 py-4 sm:py-5 overflow-y-auto max-h-[calc(100vh-210px)] sm:max-h-[70vh]">
                <form onSubmit={onSubmit} className="mt-0 space-y-4">
                  <div className="grid grid-cols-1 sm:grid-cols-2 text-[12px] sm:text-[16px] gap-4">
                    <Field label="NAME*" value={form.name} onChange={(e) => setField("name", e.target.value)} error={errors.name} />
                    <Field label="EMAIL*" type="email" value={form.email} onChange={(e) => setField("email", e.target.value)} error={errors.email} />
                  </div>
                  <div className="grid grid-cols-1 sm:grid-cols-2 text-[12px] sm:text-[16px] gap-4">
                    <Field label="PHONE*" type="tel" value={form.mobile} onChange={(e) => setField("mobile", e.target.value)} error={errors.mobile} />
                    <Field label="SUBJECT*" value={form.subject} onChange={(e) => setField("subject", e.target.value)} error={errors.subject} />
                  </div>
                  <div>
                    <label className="block text-[12px] sm:text-[16px] font-bold text-[#0B3BFF] tracking-wide">MESSAGE*</label>
                    <textarea
                      rows={4}
                      placeholder="Tell us what you need..."
                      value={form.message}
                      onChange={(e) => setField("message", e.target.value)}
                      className={["mt-2 w-full rounded-lg bg-white border px-3 py-2 text-[13px]", "text-slate-700 placeholder:text-slate-400 outline-none resize-none", errors.message ? "border-red-400" : "border-[#E5EAF6]", "focus:border-[#0B3BFF] focus:ring-2 focus:ring-[#0B3BFF]/15"].join(" ")}
                    />
                    {errors.message && <p className="mt-1 text-[12px] text-red-500">{errors.message}</p>}
                  </div>

                  {formStatus.message && (
                    <div className={["rounded-lg px-3 py-2 text-[13px]", formStatus.type === "success" ? "bg-green-50 text-green-700 border border-green-200" : "bg-red-50 text-red-700 border border-red-200"].join(" ")}>
                      {formStatus.message}
                    </div>
                  )}
                  ...
                  <motion.button
                    ...
                    className={["w-full rounded-lg py-3 text-[13px] sm:text-[16px] font-semibold", "inline-flex items-center justify-center gap-2 transition", submitting ? "bg-gray-400 text-white cursor-not-allowed" : "bg-[#0B3BFF] text-white hover:bg-[#0832DB]"].join(" ")}
                  >
...
function Field({ label, type = "text", value, onChange, error }) {
  return (
    <div>
      <label className="block text-[12px] sm:text-[16px] font-bold text-[#0B3BFF] tracking-wide">{label}</label>
      <input
        type={type}
        value={value}
        onChange={onChange}
        className={["mt-2 w-full rounded-lg bg-white border px-3 py-2 text-[13px]", "text-slate-700 placeholder:text-slate-400 outline-none", error ? "border-red-400" : "border-[#E5EAF6]", "focus:border-[#0B3BFF] focus:ring-2 focus:ring-[#0B3BFF]/15"].join(" ")}
      />
