...
import { CheckCircle2, PhoneIcon, MailIcon, ArrowLeft } from "lucide-react";
...
export default function ThankYou() {
  // Get name and phone from URL query params (passed by JobApplicationForm)
          ...
          <div className="flex flex-col gap-6">
            <div className="flex items-start gap-1 sm:gap-4">
              <div className="mt-1 flex shrink-0 rounded-xl bg-[#F6F7FF] p-2.5 sm:rounded-2xl sm:p-3">
                <CheckCircle2 size={24} className="text-[#0B3BFF] sm:[size:28]" />
              </div>
  ...
  return (
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        ...
        <div className={`mt-0.5 text-sm font-semibold text-[#0F3F3B] sm:text-[14px] sm:mt-0 ${breakAll ? "break-words" : ""}`}>
          {value}
        </div>
      </div>
    </a>
